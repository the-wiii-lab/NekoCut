	...
	ipcMain.handle("open-video-file-picker", async (_, options?: OpenVideoFilePickerOptions) => {
		try {
			...
			const result = await dialog.showOpenDialog({
				title: includeProjects ? "Import Media or NekoCut Project" : "Select Video File",
				defaultPath: recordingsDir,
				filters: [
					...(includeProjects
						? [
								{
									name: "Media or NekoCut Projects",
									extensions: [
					...
					...(includeProjects
						? [{ name: "NekoCut Projects", extensions: PROJECT_FILE_EXTENSIONS }]
						: []),
